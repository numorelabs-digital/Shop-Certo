...
function Preferences({location,postalCode,street,radius,close,save}:{location:string;postalCode:string;street:string;radius:number;close:()=>void;save:(location:string,postalCode:string,street:string,radius:number,place?:PlaceSuggestion)=>void}){
  ...
  useEffect(()=>{const closeSuggestions=(event:PointerEvent)=>{const target=event.target;if(target instanceof Element&&!target.closest(".location-autocomplete"))setSuggestions([])},closeOnEscape=(event:KeyboardEvent)=>{if(event.key==="Escape")setSuggestions([])};document.addEventListener("pointerdown",closeSuggestions);document.addEventListener("keydown",closeOnEscape);return()=>{document.removeEventListener("pointerdown",closeSuggestions);document.removeEventListener("keydown",closeOnEscape)}},[]);
