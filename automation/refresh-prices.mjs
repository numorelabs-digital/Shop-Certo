...
async function updateProduct({userId,productId,requestRef}){
  ...
  if(best){const previous=number(product.bestPrice),firstOffer=!Number.isFinite(previous),sourceOffers=unique.slice(0,30).map(item=>({url:item.url,store:item.store,title:item.title,price:item.price,...(Number.isFinite(item.shipping)?{shipping:item.shipping}:{}),...(item.imageUrl?{imageUrl:item.imageUrl}:{})}));await productRef.set({name:product.sourceUrl&&best.title?best.title:product.name,bestPrice:best.price,...(Number.isFinite(previous)?{oldPrice:previous}:Number.isFinite(best.oldPrice)?{oldPrice:best.oldPrice}:{}),shipping:Number.isFinite(best.shipping)?best.shipping:0,store:best.store,offerUrl:best.url,imageUrl:best.imageUrl||product.imageUrl||null,sourceOffers,offersCount:unique.length,monitoring:true,priceStatus:"ready",lastPriceCheckAt:FieldValue.serverTimestamp(),lastPriceError:FieldValue.delete(),locationUsed:{postalCode:user.postalCode||null,street:user.street||null,radiusKm:user.radiusKm||null}}, {merge:true});await productRef.collection("priceHistory").add({price:best.price,store:best.store,url:best.url,checkedAt:FieldValue.serverTimestamp()});if(firstOffer){const eventRef=db.doc(`users/${userId}/alertEvents/automatic_${productId}_${best.price.toFixed(2).replace(".","-")}`);if(!(await eventRef.get()).exists)await eventRef.set({alertId:"automatic",productId,productName:product.name,message:"Encontramos a primeira oferta deste produto",price:best.price,store:best.store,offerUrl:best.url,createdAt:FieldValue.serverTimestamp()})}if(requestRef)await requestRef.set({status:"done",offersFound:unique.length,processedAt:FieldValue.serverTimestamp()},{merge:true});return true}
  await productRef.set({monitoring:true,priceStatus:"no_offer",lastPriceCheckAt:FieldValue.serverTimestamp(),lastPriceError:errors.slice(0,6),locationUsed:{postalCode:user.postalCode||null,street:user.street||null,radiusKm:user.radiusKm||null}},{merge:true});if(requestRef)await requestRef.set({status:"retry",attempts:FieldValue.increment(1),lastErrors:errors.slice(0,6),processedAt:FieldValue.serverTimestamp()},{merge:true});return false;
}
